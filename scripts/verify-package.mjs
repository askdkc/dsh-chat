...
execFileSync(process.execPath, ['scripts/verify-core-patch.mjs'], { stdio: 'inherit' })
...
for (const path of ['lib/index.js', 'lib/client.js', 'lib/types/index.d.ts', 'lib/types/client/index.d.ts', 'cordis.patch.yml', 'patches/dsh-d347e703-hero-actions.patch', 'README.md', 'README.ja.md', 'README.zh.md', 'LICENSE']) assert(existsSync(path), `Missing ${path}`)
const packing = JSON.parse(execFileSync('npm', ['pack', '--dry-run', '--json'], { encoding: 'utf8' }))[0]
for (const { path } of packing.files) {
  assert(/^(lib\/|patches\/|docs\/|README(?:\.ja|\.zh)?\.md$|LICENSE$|package\.json$|cordis\.patch\.yml$)/.test(path), `Unexpected published file ${path}`)
